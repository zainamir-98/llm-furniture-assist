...
def feature_extractor(img=None, img_path=None, model=None,
...
if __name__ == "__main__":
    ...
    # load cropped image
    image_path = os.path.join("activity recognition", "mock data", "cropped", "rabbit_cropped.jpg")
    ...
    with torch.no_grad():
        features = resnet(tensor_image.unsqueeze(0))

    # we can use the extracted features for the activity recognition
    print(features.shape)

    print(feature_extractor(img_path=image_path).shape)
